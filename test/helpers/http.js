...
function withFakeServer(routes, block) {
  ...
  describe('', () => {
    ...
    beforeEach(function() {
      ...
      if (!httpSpy) {
        httpSpy = sinon.stub(http, 'request').callsFake(fakeRequestMethod(router));
        httpsSpy = sinon.stub(https, 'request').callsFake(fakeRequestMethod(router));
      }
    });
